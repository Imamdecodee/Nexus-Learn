(async function () {
  ...
  const leadership = window.CAMPUS_ID === 'unilorin' ? [
    { name:'Bakare Aminat Adewunmi', role:'Campus President · Hub Manager', meta:'Employment Relations & HRM · 200 Level', contact:'+2349057808154', hobbies:'Community building, learning', image:'president.jpg', vision:'I am glad to have everyone onboard and I look forward to the growth and progress of ILM NEXUS with every single person.' },
  ...
  const excludedLeadershipNames = new Set(['muhammad bada']);
  const classroom = [
  ...
  app.innerHTML = `
    ...
    <main>
      ...
      <section class="section" id="leadership"><div class="wrap"><p class="eyebrow blue">Executive roster</p><h2>Meet the ${escapeHtml(campus.name)} chapter leadership team.</h2><p class="lead">A student-led leadership team creating a more capable, connected, and opportunity-ready campus community.</p><div class="leaders-grid">${leadership.length ? leadership.map((person, index) => `<article class="leader-card ${index === 0 ? 'featured-leader' : ''}"><img src="${root}state/kwara/unilorin/staffpic/${encodeURIComponent(person.image)}" alt="${escapeHtml(person.name)}"><span class="leader-role">${escapeHtml(person.role)}</span><h3>${escapeHtml(person.name)}</h3><p class="leader-meta">${escapeHtml(person.meta)}</p><p><strong>Contact:</strong> <a class="text-link" href="tel:${person.contact}">${escapeHtml(person.contact)}</a></p><p><strong>Hobbies:</strong> ${escapeHtml(person.hobbies)}</p><p class="vision">${escapeHtml(person.vision)}</p></article>`).join('') : '<article class="empty-state"><i class="fas fa-users"></i><p>Chapter leadership profiles will appear here when assigned by central administration.</p></article>'}</div></div></section>
  ...
  async function syncQueuedFeedback(db, firestoreModule) {
    const queued = JSON.parse(localStorage.getItem('campus_feedback') || '[]');
    if (!queued.length) return;
    const { addDoc, collection, serverTimestamp } = firestoreModule;

    for (const item of queued) {
      try {
        await addDoc(collection(db, 'campus_feedback'), { ...item, createdAt: serverTimestamp() });
      } catch (error) {
        console.warn('Queued feedback could not be synced:', error);
        return;
      }
    }

    localStorage.removeItem('campus_feedback');
  }
  ...
  async function connectFirebase() {
    try {
      ...
      await syncQueuedFeedback(db, firestoreModule);
      ...
      onSnapshot(collection(db, 'campuses', 'unilorin', 'staff_profiles'), snapshot => {
        ...
        if (!grid) return;
        const staticNames = new Set(leadership.map(person => String(person.name || '').trim().toLowerCase()));
        const remoteProfiles = snapshot.docs
          .map(item => item.data())
          .filter(person => {
            const name = String(person.name || '').trim();
            const normalized = name.toLowerCase();
            return !!name && !staticNames.has(normalized) && !excludedLeadershipNames.has(normalized);
          });
