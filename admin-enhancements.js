...
function addLiveMetrics() {
  ...
  onSnapshot(collection(db, 'users'), snapshot => { update('studentMetric', snapshot.size); update('pendingMetric', snapshot.docs.filter(item => item.data().projectReviewStatus !== 'Approved').length); const body = $('studentDetailsBody'); if (body) body.innerHTML = snapshot.docs.map(item => { const student = item.data(); return `<tr><td style="padding:.55rem;border-bottom:1px solid var(--line)">${student.name || 'Unnamed'}</td><td style="padding:.55rem;border-bottom:1px solid var(--line)">${student.email || ''}</td><td style="padding:.55rem;border-bottom:1px solid var(--line)">${student.phone || 'Not provided'}</td><td style="padding:.55rem;border-bottom:1px solid var(--line)">${student.course || 'Not assigned'}</td><td style="padding:.55rem;border-bottom:1px solid var(--line)">${student.institution || student.campusId || 'Not provided'}</td><td style="padding:.55rem;border-bottom:1px solid var(--line);color:var(--green);font-weight:700">${student.approvalStatus || student.projectReviewStatus || 'APPROVED'}</td></tr>`; }).join('') || '<tr><td colspan="6" style="padding:.7rem;color:var(--muted)">No students found.</td></tr>'; }, () => {});
